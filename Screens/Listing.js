...
import {Button,Share,View,StyleSheet,ScrollView,TouchableOpacity,Modal,Alert,Image} from 'react-native';
         ...
         {sd.map(item=>(
          <View key={item.id}>
           <View style={styles.nav}>
             <TouchableOpacity 
             style={{marginLeft:20}}
             onPress={()=> clos()}><Icon name="angle-left" size={40} color="white"/>
             </TouchableOpacity>

             <View style={{position:'absolute',right:20,top:15,flexDirection: 'row',}}>
             <TouchableOpacity 
             style={{marginRight:20}}
             onPress={()=> onShar(item.hadith)}
